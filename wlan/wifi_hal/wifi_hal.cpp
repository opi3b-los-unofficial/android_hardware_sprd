    ...
    *set = 0;
    ...
    if (ret != WIFI_SUCCESS) {
        *set = info->supported_feature_set;
        ALOGV("Supported feature set acquired at initialization : %x", (unsigned int)*set);
    } else {
        info->supported_feature_set = *set;
        ALOGV("Supported feature set acquired : %x", (unsigned int)*set);
    }
    return WIFI_SUCCESS;
}
...
cleanup:
    /*
     * coverity bug1159023:
     * vCommand alway be allocated as initialize_vendor_cmd() return WIFI_SUCCESS.
     * if (vCommand)
     */
        delete vCommand;
    return ret;
