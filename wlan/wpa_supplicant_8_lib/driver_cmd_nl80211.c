...
#define SOFTAP_SUPPORT_5G "ro.wifi.softap.support5G"

int global_sigma_mode = 0;
...
#define HOSTAP_BLOCK_LIST_FILE "/data/vendor/wifi/hostapd/hostapd.blocklist"

/*
 * define max stations according to the CP2 capacity(BUG:839423):
 * WCN chip Max num sta
 * 2351 8
 * Pike2 10
 * Sharkle 10
 * Sharklep 10
 ...
 * Marlin3 10
 */
#define HOSTAP_MAX_BLOCK_NUM (10)
...
//<-- Add for SoftAp Advance Feature END
    ...
    if ((ret = ioctl(drv->global->ioctl_sock, SIOCDEVPRIVATE + 2, &ifr)) < 0) {
      ...
      wpa_printf(MSG_INFO, "%s %s len = %d, %zu", __func__, buf, ret,
                 strlen(buf));
    }
...
//NOTE: Bug#766775 Add for VTS test BEG-->
    if (strstr(ifr.ifr_name,"p2p-dev")) {
        wpa_printf(MSG_DEBUG, "MIRACAST: convert to wlan0 interface");
        memset(ifr.ifr_name, 0, IFNAMSIZ);
        os_strlcpy(ifr.ifr_name, "wlan0", IFNAMSIZ);
    }
//<-- Add for VTS test END
    if ((ret = ioctl(drv->global->ioctl_sock, SIOCDEVPRIVATE + 2, &ifr)) < 0) {
      wpa_printf(
          MSG_ERROR,
    ...
    wpa_printf(MSG_DEBUG, "%s: set P2PMACADDR\n", __func__);
    memset(&ifr, 0, sizeof(ifr));
    memset(&priv_cmd, 0, sizeof(priv_cmd));
    os_strlcpy(ifr.ifr_name, "wlan0", IFNAMSIZ);
    ...
    os_memcpy(cmd_buf, cmd, sizeof(cmd_buf));
#ifdef USE_64_BIT_IPC
    priv_cmd.buf = (u64)(uintptr_t)cmd_buf;
#else
    priv_cmd.buf = cmd_buf;
#endif
    ...
    if ((ret = ioctl(drv->global->ioctl_sock, SIOCDEVPRIVATE + 6, &ifr)) < 0) {
        wpa_printf(MSG_ERROR, "%s: failed to issue private commands(%s), total_len=%d, addr = " MACSTR "\n",
                 __func__, strerror(errno), priv_cmd.total_len, MAC2STR(cmd_buf + 11));
        ret = -1;
    } else {
        wpa_printf(MSG_DEBUG, "%s: Success to set P2PMACADDR " MACSTR, __func__, MAC2STR(cmd_buf + 11));
    }
    ...
    os_memset(cmd_buf, 0, sizeof(cmd_buf));
    os_strlcpy(cmd_buf, cmd, sizeof(cmd_buf));
//NOTE: Bug#474462 Add for SoftAp Advance Feature BEG-->
#ifdef USE_64_BIT_IPC
    priv_cmd.buf = (u64)(uintptr_t)cmd_buf;
#else
...
//<-- Add for SoftAp Advance Feature END
    ...
    if ((ret = ioctl(drv->global->ioctl_sock, SIOCDEVPRIVATE + 4, &ifr)) < 0) {
      wpa_printf(MSG_ERROR, "%s: failed to issue private commands(%s)， total_len=%d\n",
                 __func__, strerror(errno), priv_cmd.total_len);
      ret = -1;
    }
    ...
    if (valid) {
        ...
        if ((ret = wpa_drv_set_country(wpa_s, pos)) < 0) {
            wpa_printf(MSG_ERROR, "%s: failed to set country code: %s\n",
...
//<-- Add for SoftAp Advance Feature END
    ...
    if ((ret = ioctl(drv->global->ioctl_sock, SIOCDEVPRIVATE + 1, &ifr)) < 0) {
      ...
      wpa_printf(MSG_DEBUG, "%s %s len = %d, %zu", __func__, buf, ret,
                 strlen(buf));
...
//NOTE: Bug#474462 Add for SoftAp Advance Feature BEG-->
	...
	if (pos == NULL)
        ...
        wpa_printf(MSG_DEBUG, "set sigma mode: %d: 0x%p", sigma, wpa_s->global);
    } else { /* Use private command */
        wpa_printf(MSG_ERROR, "Unknown driver cmd: %s", cmd);
    }
    return ret;
}
...
int driver_init_mac_acl(struct hostapd_data *hapd)
{
    ...
    char reply[10];
    os_memset(buf, 0, 25);
    strncpy((char *)buf, "WHITE_ADD_ONCE ", 15);
