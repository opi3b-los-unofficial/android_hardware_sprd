...
int userial_vendor_open(tUSERIAL_CFG *p_cfg)
{
    ...
    ALOGI("userial vendor open: opening %s, baud %d", vnd_userial.port_name,
          p_cfg->baud);

    if ((vnd_userial.fd = open(vnd_userial.port_name, O_RDWR)) == -1) {
        ALOGE("userial vendor open: unable to open %s errno %d %s", vnd_userial.port_name, errno, strerror(errno));
        return -1;
    }
    ...
    if (property_get("ro.build.type", value, "") >= 0 && strstr(value, "userdebug")) {
        ALOGI("%s set usrdebug mode to tty.", __func__);
        int fd = -1, ret;
        char value_usr_debug = 2;
        //write to sipc node
        fd = open(SPIC_SYSFS_DUMPMEM_NODE, O_WRONLY);
        if (fd < 0) {
          ALOGI("open %s faild, error=%s (%d)", SPIC_SYSFS_DUMPMEM_NODE, strerror(errno), errno);
        } else {
          ret = write(fd, &value_usr_debug, 1);
          if (ret < 0) {
            ALOGI("write %s faild, error=%s (%d)", SPIC_SYSFS_DUMPMEM_NODE, strerror(errno), errno);
          }
          close(fd);
        }
        //write to sdio node
        ...
        if (fd < 0) {
          ALOGI("open %s faild, error=%s (%d)", SDIO_SYSFS_DUMPMEM_NODE, strerror(errno), errno);
        } else {
          ret = write(fd, &value_usr_debug, 1);
          if (ret < 0) {
            ALOGI("write %s faild, error=%s (%d)", SDIO_SYSFS_DUMPMEM_NODE, strerror(errno), errno);
          }
          close(fd);
        }
    }
...
void userial_vendor_close(void)
{
    ...
    result = tcgetattr(vnd_userial.fd, &term);
    if (result < 0) {
        ALOGE("tcgetattr(fd:%d) FAILED result:%d", vnd_userial.fd, result);
    } else {
        term.c_cflag &= ~(CRTSCTS);
        ALOGI("device fd = %d CRTSCTS clear", vnd_userial.fd);
        result = tcsetattr(vnd_userial.fd, TCSANOW, &term);
        if (result < 0) {
            ALOGE("tcsetattr(fd:%d) FAILED result:%d", vnd_userial.fd, result);
        }
    }

    result = close(vnd_userial.fd);
    if (result < 0) {
        ALOGE("close(fd:%d) FAILED result:%d", vnd_userial.fd, result);
    }
    ALOGI("device fd = %d clear", vnd_userial.fd);
...
{
    snprintf(vnd_userial.port_name, VND_PORT_NAME_MAXLEN, "%s", p_conf_value);
    UNUSED(p_conf_name);
    UNUSED(param);
    return 0;
}
